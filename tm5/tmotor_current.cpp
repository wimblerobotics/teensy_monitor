...
int TMotorCurrent::GetValueMa(Motor index) {
  if (index == kLeft) return g_left_motor_current_ma_;
  else return g_right_motor_current_ma_;
}


void TMotorCurrent::loop() {
  if (g_have_left_motor_sensor_) {
    g_left_motor_current_ma_readings_[g_next_motor_current_index_] = g_left_INA260_.readCurrent();
    char msg[512];
    snprintf(msg, sizeof(msg), "LCurrent: %-2.3f", g_left_motor_current_ma_readings_[g_next_motor_current_index_]);
  ...
  if (g_have_right_motor_sensor_) {
    g_right_motor_current_ma_readings_[g_next_motor_current_index_] = g_right_INA260_.readCurrent();
  } else {
